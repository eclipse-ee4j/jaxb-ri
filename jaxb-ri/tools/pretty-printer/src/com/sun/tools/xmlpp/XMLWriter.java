...

	...
	throws SAXException
    {
        if(startTagClosePending) {
            startTagClosePending = false;
            write(" />");
        } else {
            write("</");
            writeName(uri, localName, qName, true);
...

    ...
    private void write (char[] ch, int start, int len) throws SAXException {
        checkStartTagClose();
        try {
    ...
    private void checkStartTagClose() throws SAXException {
        if( !startTagClosePending ) return;
        startTagClosePending = false;
        write('>');
    }
...

    ...
    private boolean startTagClosePending = false;
}
